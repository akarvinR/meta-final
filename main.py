...
import subprocess
import os
ROOT_DIRECTORY = Path("/code_execution/VCSL")
DATA_DIRECTORY = Path("/data")
QRY_VIDEOS_DIRECTORY = DATA_DIRECTORY / "query"
OUTPUT_FILE = Path("/code_execution")/ "subset_query_descriptors.npz"
...
def main():
    ...
    s = " conda run --no-capture-output -n condaenv pip install git+https://github.com/lyakaap/ISC21-Descriptor-Track-1st"
    os.system(s)
    s = " conda run --no-capture-output -n condaenv python -m generate_dataframe.py --outputfile subset.csv --path " + str(QRY_VIDEOS_DIRECTORY)
    os.system(s)
    s = " conda run --no-capture-output -n condaenv python -m reate_desc.py --outputfile" + str(OUTPUT_FILE) +"--csvfile subset.csv  --acc cuda --batchsize 10 --numwork 1"
    os.system(s)


    s = '''    conda run --no-capture-output -n condaenv python -m vsc.baseline.inference
